...
/// Resolve an installation folder without creating it and reject locations
...
fn canonical_install_path(path: &Path) -> Result<PathBuf, String> {
    let mut candidate = if path.is_absolute() {
        ...
        env::current_dir()
            .map_err(|e| format!("Resolve installation folder: {e}"))?
            .join(path)
    };
        ...
        match fs::symlink_metadata(&candidate) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                let component = candidate
                    .file_name()
                    .ok_or("Installation folder has no usable path")?;
                missing.push(component.to_owned());
                candidate = candidate
                    .parent()
                    .ok_or("Installation folder has no existing parent")?
                    .to_path_buf();
            }
            Err(error) => return Err(format!("Resolve installation folder: {error}")),
        }
    }
    let mut resolved = fs::canonicalize(&candidate)
        .map_err(|e| format!("Resolve installation folder {}: {e}", candidate.display()))?;
...
pub fn cli_main() -> Result<(), String> {
    if std::env::args().nth(1).as_deref() == Some("download-probe") {
        return http::download_probe();
    }
